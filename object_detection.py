...
global existingOutputs, imgCounter
imgCounter = 0
existingOutputs = []

def predict_other_voileneces(ImageFileName):
    ...
    rid_im0 = cv2.imread(ImageFileName)
    cv2.imwrite('output/Det_'+str(imgCounter)+'.png',rid_im0)

    existingOutputs.append(str("None")+'\n'+str("None")+'\n'+str(rider_lp_number)+'\n'+str("None")+"\nSignal Breaked")
...
def draw_boxes(image, boxes, line, labels, obj_thresh, dcnt):
    ...
    for box in boxes:
        ...
        if label >= 0:
            ...
            cv2.line(image, line[0], line[1], (255, 0, 0), 1)

            if tf:
                cv2.rectangle(image, (box.xmin,box.ymin), (box.xmax,box.ymax), (255,0,0), 1)
                cimg = plane_img[box.ymin-30:box.ymax+30, box.xmin-30:box.xmax+30]
                cv2.imshow("violation", cimg)
                cv2.waitKey(1)
                cv2.imwrite("Detected Images/violation_"+str(dcnt)+".jpg", cimg)
                Thread(target=predict_other_voileneces,args=("Detected Images/violation_"+str(dcnt)+".jpg",)).start()
...
weights_path = r"C:\Users\iamsa\Downloads\yolov3.weights"
# set some parameters
net_h, net_w = 416, 416
